...
void Train()
{
    ...
    std::ofstream loss_file("epoch_loss.txt");
    if (!loss_file) {
        std::cerr << "Error abriendo el archivo para escribir los datos de perdida." << std::endl;
        return;
    }
    ...
    for (int epoch = 1; epoch <= num_epochs; epoch++) {
        ...
        printf("Epoca %3d, porcentage de perdida=%.6f\n", epoch, epoch_loss);

        // Escribir los datos en el archivo
        loss_file << epoch << "\t" << epoch_loss << std::endl;
    }
